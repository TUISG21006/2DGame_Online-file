//GUI設定の読み込み
const Map= require('./map.json').map; 
const Init = require('./data.json'); 
var f = require('./MoveFunc.js');
//f.func();
...
//サーバ内アイテム管理処理（クライアント同期の際はclearされる）
let ServerItem = setInterval (()=>{
    if(Init.Synchro_Item == Client)clearInterval(ServerItem);
    io.sockets.emit('Synchronize_Item',ItemData);
},1000/Init.fps);
